...
- **Bookmarks**: tap the bookmark icon (or press **B**) to save the page. Bookmarks are named after the clause or table on the page, and you can rename them. Use **Saved → Copy saved items** to move bookmarks, groups and highlights to another device.
...
- **Groups and highlighter**: in **Saved**, create colour-coded groups (for example "RCD rules" or "Bathroom zones"), then rename, recolour or delete them. Select text in the PDF, then right-click (PC) or use the pop-up bar (phone) to highlight it in a group's colour. The highlight is saved in that group, and tapping it later lets you move it to another group, copy it or remove it. Bookmarks can be moved into groups too, and new bookmarks go into the last group you used.
...
- **Guides**: step-by-step checklists for on-site jobs. Tick steps as you go, write down readings (insulation resistance goes green or red against its minimum), jump to the Zs check or the linked clauses, then use **Copy results** to paste into your job notes or certificate. Progress is saved on the device for each guide. The first guide is **EV charger testing and certification (NZ)**. To add a guide, add an entry to `guides.js`.
